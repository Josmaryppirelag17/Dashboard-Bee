...
export const FocusTimer: React.FC<FocusTimerProps> = ({ onSessionComplete }) => {
  ...
  const [completionPending, setCompletionPending] = useState<{ taskId: string; taskTitle: string } | null>(null);
  ...
  useEffect(() => {
    if (selectedTaskId && !storeTasks.find((t) => t.id === selectedTaskId && t.columnId !== "completed")) {
  ...
  const confirmCompletion = async (completed: boolean) => {
    ...
    if (completed) {
      ...
      showToast(language === "en" ? "Task moved back to pending" : "Tarea devuelta a pendiente", "info");
    }
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch relative">
      ...
      <div className="lg:col-span-5 bg-white/75 backdrop-blur-md border border-[#ebdcb9]/60 rounded-3xl p-6 shadow-[0_10px_35px_rgba(110,80,45,0.04)] flex flex-col justify-between items-center relative overflow-hidden text-center min-h-[420px]">
        ...
        <div className="w-full relative z-10">
          ...
          <h3 className="text-md font-bold text-[#100f0d]">{language === 'en' ? 'Foraging Rhythm' : 'Ritmo de Polinización'}</h3>
          ...
          <div className="flex flex-col items-center space-y-3 bg-[#faf6ee]/70 border border-[#ebdcb9]/40 p-3.5 rounded-2xl max-w-xs mx-auto mb-2">
            <div className="flex flex-col space-y-1.5 w-full">
              <div className="flex items-center justify-between w-full">
                <span className="text-[10px] font-bold uppercase tracking-wider text-[#5c5449]">{t.timer.focusTag}</span>
                <span className="text-xs font-mono font-bold text-[#e28800]">{workMinutes} min</span>
              </div>
              <div className="flex items-center space-x-1.5 w-full">
                {[15, 25, 45, 60].map((mins) => (
                  ...
                  <input
                    ...
                    aria-label={language === 'en' ? 'Custom focus minutes' : 'Minutos de foco personalizados'}
                    title={language === 'en' ? 'Custom focus minutes' : 'Minutos de foco personalizados'}
                  />
              </div>
            </div>
            ...
            <div className="flex flex-col space-y-1.5 w-full border-t border-[#ebdcb9]/20 pt-2.5">
              <div className="flex items-center justify-between w-full">
                <span className="text-[10px] font-bold uppercase tracking-wider text-[#5c5449]">{t.timer.breakTag}</span>
                <span className="text-xs font-mono font-bold text-emerald-700">{breakMinutes} min</span>
              </div>
              <div className="flex items-center space-x-1.5 w-full">
                {[3, 5, 10, 15].map((mins) => (
                  <button
                  ...
                  <input
                    ...
                    aria-label={language === 'en' ? 'Custom break minutes' : 'Minutos de descanso personalizados'}
                    title={language === 'en' ? 'Custom break minutes' : 'Minutos de descanso personalizados'}
                  />
              </div>
            </div>
          </div>
        ...
        <div className="my-6 relative w-44 h-44 flex items-center justify-center select-none group" role="timer" aria-live="polite" aria-label="Temporizador Pomodoro">
          ...
          <div className="text-center relative z-10 transition-transform duration-300 group-hover:scale-105">
            ...
            <span className="text-[9.5px] uppercase font-bold tracking-widest text-[#5c5449] mt-1 pr-0.5 inline-block">
              {isBreak ? (language === 'en' ? 'RELAX' : 'Relajarse') : (language === 'en' ? 'FOCUS' : 'Foco Activado')}
            </span>
          </div>
        </div>
        ...
        <div className="w-full relative z-10">
          <div className="flex items-center justify-center space-x-3 mb-4">
            ...
            <button
              ...
              aria-label={soundEnabled ? (language === 'en' ? 'Mute sound' : 'Silenciar sonido') : (language === 'en' ? 'Enable sound' : 'Activar sonido')}
              title={soundEnabled ? (language === 'en' ? 'Bee buzz enabled' : 'Zumbido abeja activado') : (language === 'en' ? 'Muted' : 'Silenciado')}
            >
          ...
          <div className="w-full bg-[#faf6ee]/80 border border-[#ebdcb9] rounded-2xl px-3.5 py-2">
            ...
            {availableTasks.length === 0 ? (
              <span className="block text-xs text-[#5c5449] font-medium text-center italic">
                {language === 'en' ? 'No pending tasks — add one in the Kanban board' : 'Sin tareas pendientes — agrega una en el Kanban'}
              </span>
            ) : (
              <select
              ...
              >
                <option value="">{language === 'en' ? '-- Select a task to auto-complete --' : '-- Elegir labor para completar --'}</option>
                {availableTasks.map((task) => (
                  <option key={task.id} value={task.id}>
                    {task.title} ({task.columnId === 'in_progress' ? (language === 'en' ? 'in progress' : 'en proceso') : (language === 'en' ? 'pending' : 'pendiente')})
                  </option>
                ))}
              </select>
            )}
      ...
      <div className="lg:col-span-7 bg-white/75 backdrop-blur-md border border-[#ebdcb9]/60 rounded-3xl p-6 shadow-[0_10px_35px_rgba(110,80,45,0.04)] flex flex-col justify-between min-h-[420px]">
        ...
        <div className="flex items-center justify-between mb-4.5 flex-shrink-0">
          <div>
            <h3 className="text-md font-bold text-[#100f0d]">{language === 'en' ? 'Focus Cells' : 'Celda de Foco'}</h3>
            <p className="text-xs text-[#5c5449]">{language === 'en' ? 'Click any cell to log immediate focus or complete pomodoros' : 'Haz clic en una celda para registrar foco inmediato o completa pomodoros'}</p>
          </div>
          <div className="flex items-center space-x-1 bg-[#faf6ee] px-2.5 py-1.5 rounded-xl border border-[#ebdcb9]/60 font-mono text-[11px] font-bold text-[#e28800]">
            <span>{cells.filter(c => c.completed).length}/{cells.length}</span>
            <span className="text-[10px] text-[#5c5449] font-sans">{language === 'en' ? 'cells ready' : 'celdas listas'}</span>
          </div>
        </div>
        ...
        <div className="flex flex-col items-center justify-center flex-1 py-3 my-4 overflow-y-auto">
          {cells.length === 0 ? (
            <div className="text-center py-10 text-[#5c5449]">
              <p className="text-xs font-bold">{language === 'en' ? 'No tasks yet — head to the Kanban board to add some!' : 'Aún no hay labores — ¡ve al Kanban para agregar!'}</p>
            </div>
          ) : (
                      ...
                      <div className="w-24 h-[105px] relative flex items-center justify-center">
                        ...
                        <div className="relative z-10 text-center px-2 select-none">
                          {cell.completed ? (
                            ...
                            <span className="text-[#e28800] font-mono text-[11px] font-bold">►</span>
                          ) : null}
                          <span className={`block text-[8.5px] truncate max-w-[70px] mt-1 ${cell.completed ? 'text-white font-extrabold' : isSelected ? 'text-[#e28800] font-extrabold' : 'text-[#5c5449]'}`}>
                            {cell.title}
                          </span>
                        </div>
                      </div>
        ...
        <div className="pt-4 border-t border-[#ebdcb9]/45 flex items-center justify-between text-xs text-[#5c5449] flex-shrink-0">
          <span className="flex items-center space-x-1.5">
            ...
            <span>{language === 'en' ? 'Work Colony' : 'Colonia de Trabajo'}: <strong className="text-[#e28800] font-bold">{language === 'en' ? 'Active Worker' : 'Obrera Activa'}</strong></span>
          </span>
