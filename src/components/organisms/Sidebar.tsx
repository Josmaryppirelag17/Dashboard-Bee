...
const BEE_PREFIXES_ES = ['Reina', 'Obrera', 'Abeja', 'Zángano', 'Exploradora', 'Néctar', 'Zumbadora', 'Larva'];
const BEE_SUFFIXES_ES = ['Mellifera', 'Dorada', 'Veloz', 'Productiva', 'Trabajadora', 'Constante', 'Gladiadora', 'DeElite'];

const BEE_PREFIXES_EN = ['Queen', 'Worker', 'Bee', 'Drone', 'Forager', 'Nectar', 'Buzzer', 'Larva'];
const BEE_SUFFIXES_EN = ['Mellifera', 'Golden', 'Swift', 'Productive', 'Determined', 'Steady', 'Gladiator', 'Elite'];
...
export default function Sidebar({ 
  ...
  const getMenuIcon = (name: string, active: boolean) => {
    const iconClass = `w-5 h-5 transition-transform duration-300 group-hover:scale-110 ${
      active ? 'text-[#e28800] filter drop-shadow-[0_1.5px_3px_rgba(226,136,0,0.2)]' : 'text-[#5c5449]'
    }`;
    switch (name) {
      case 'panel': return <LayoutDashboard className={iconClass} />;
      case 'focus': return <Flame className={iconClass} />;
      case 'analytics': return <TrendingUp className={iconClass} />;
  ...
  return (
    ...
    >
      ...
      <div>
        <div className={`p-6 flex items-center ${isCollapsed ? 'justify-center' : 'space-x-3'} border-b border-[#ebdcb9]/40`}>
          <div className="relative w-8 h-8 flex items-center justify-center shrink-0">
            {/* Double layered hexagon logo */}
            <svg viewBox="0 0 100 100" className="absolute inset-0 w-full h-full fill-[#e28800] animate-pulse" aria-hidden="true">
              <path d="M 50 0 L 93 25 L 93 75 L 50 100 L 7 75 L 7 25 Z" fillOpacity="0.12" stroke="#e28800" strokeWidth="4.5" />
            </svg>
          ...
          {!isCollapsed && (
            <div className="animate-fade-in flex-1">
              <h1 className="text-sm font-black tracking-wider text-[#100f0d] uppercase flex items-center gap-1">
                BeeHive <Sparkles className="w-3.5 h-3.5 text-[#e28800] animate-spin" style={{ animationDuration: '6s' }} aria-hidden="true" />
              </h1>
        ...
        {/* Navigation Options */}
        <nav className="p-4 space-y-2 mt-4" aria-label={language === 'en' ? 'Main navigation' : 'Navegación principal'}>
          {menuItems.map((item) => {
              ...
              >
                ...
                {isCollapsed && (
                  <div className="absolute left-[76px] hidden group-hover:block bg-white text-[#100f0d] border border-[#ebdcb9] text-[10px] font-bold px-2.5 py-1.5 rounded-xl whitespace-nowrap shadow-md z-50 pointer-events-none">
                    {item.label}
                    {item.badge && <span className="ml-1.5 text-[8.5px] bg-[#e28800] text-white px-1 py-0.5 rounded font-bold">LIVE</span>}
                  </div>
                )}
              </button>
            );
      ...
      <div className="p-4 border-t border-[#ebdcb9]/40 space-y-4">
        ...
        {/* User Card */}
        <div className={`p-2.5 rounded-2xl bg-white/60 border border-[#ebdcb9]/50 flex items-center ${isCollapsed ? 'justify-center' : 'space-x-3'}`}>
          <div className="relative w-8.5 h-8.5 rounded-xl bg-[#faf6ee] border border-[#e28800]/25 flex items-center justify-center text-[#e28800] shrink-0">
            ...
            <div className="absolute -top-1 -right-1 w-3 h-3 bg-[#e28800] border-2 border-white rounded-full animate-bounce" title={language === 'es' ? 'Apicultor En Línea' : 'Beekeeper Online'} aria-hidden="true" />
          </div>
            ...
            <div className="min-w-0 flex-1 animate-fade-in select-none">
              <span className="text-[9px] font-black text-[#5c5449] flex items-center justify-between uppercase tracking-wide">
                ...
                <button 
                  ...
                  aria-label={language === 'en' ? 'Randomize bee name' : 'Nombre de abeja aleatoria'}
                  title={language === 'en' ? 'Randomize bee name' : 'Nombre de abeja aleatoria'}
                >
                  <Dice5 className="w-3 h-3" />
                </button>
              </span>
              <span className="text-[10px] text-[#e28800] block truncate font-mono font-bold" title={userBeeName}>
                {userBeeName}
              </span>
            </div>
          )}
