...
const KanbanColumn: React.FC<{
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3.5 pb-2 border-b border-[#ebdcb9]/30">
        <div className="flex items-center space-x-2">
          <span className={`w-2.5 h-2.5 rounded-full ${
            column.id === 'todo' ? 'bg-[#faa715] animate-pulse' :
            column.id === 'in_progress' ? 'bg-[#e28800]' : 'bg-emerald-600'
          }`} />
      ...
      <p className="text-[10px] text-[#5c5449] mb-4 italic">
        {column.label}
      </p>
...
export const TaskBoard: React.FC = () => {
  ...
  const handleFormSubmit = (data: TaskFormValues) => {
    ...
    showToast(language === 'en' ? 'New task submitted to the hive successfully!' : '¡Nueva labor ingresada al panal con éxito!', 'honey');
    reset();
    setIsFormOpen(false);
  };
    ...
    <div className={THEME_CONFIG.components.glassCard}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 flex-shrink-0 border-b border-[#ebdcb9]/30 pb-4">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-xs text-[#5c5449]">
              {language === 'en' ? 'Plan your day by dragging tasks between cells' : 'Planifica tu jornada arrastrando tareas entre las celdas'}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-3 self-end sm:self-auto">
          ...
          <HexButton
            ...
            size="sm"
          >
            {isFormOpen ? (language === 'en' ? 'Close' : 'Cerrar') : (
      ...
      <AnimatePresence>
        {isFormOpen && (
          <motion.form
            ...
            transition={{ duration: THEME_CONFIG.motion.duration, ease: THEME_CONFIG.motion.easeSpring }}
            onSubmit={handleSubmit(handleFormSubmit)}
            className="overflow-hidden mb-6 p-4 border border-[#ebdcb9] bg-[#faf6ee]/65 rounded-2xl flex flex-col space-y-4"
          >
            <div>
              <label htmlFor="task-description" className="block text-[10px] font-bold text-[#5c5449] uppercase tracking-wider mb-1.5">
                {t.kanban.formTitle}
              </label>
              <input
                id="task-description"
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label htmlFor="task-priority" className="block text-[10px] font-bold text-[#5c5449] uppercase tracking-wider mb-1.5">
              ...
              <div>
                <label htmlFor="task-category" className="block text-[10px] font-bold text-[#5c5449] uppercase tracking-wider mb-1.5">
              ...
              <div>
                ...
                <Controller
                  ...
                  name="pollenValue"
                  render={({ field }) => (
                    <div className="flex items-center space-x-1.5 h-[34px]" role="group" aria-label="Selector de esfuerzo de polen">
                      {[1, 2, 3, 4, 5].map((u) => (
                        <button
                          type="button"
                          key={u}
                          ...
                          aria-label={language === 'en' ? `${u} of 5 workload cells` : `${u} de 5 celdas de esfuerzo`}
                        >
                          <svg
                             width="18"
                             height="18"
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-5 pb-3">
        ...
        <div className="relative flex items-center bg-white border border-[#ebdcb9] px-3.5 py-1.5 rounded-xl text-xs text-[#5c5449] hover:border-[#e28800] transition-colors shadow-sm cursor-pointer min-w-[190px]">
          <label htmlFor="taskboard-category-select" className="font-bold mr-1.5">{language === 'en' ? 'Category:' : 'Categoría:'}</label>
          <select
          ...
          <ChevronDown className="w-3.5 h-3.5 text-stone-400 absolute right-3 pointer-events-none" aria-hidden="true" />
        </div>
      </div>
      ...
      <DndContext sensors={sensors} onDragEnd={handleDragEnd}>
        <div className="w-full overflow-x-auto pb-4 custom-scrollbar">
          <div className="flex flex-col lg:flex-row gap-5 items-stretch lg:min-w-[960px] xl:min-w-full">
            {[
              { id: 'todo' as ColumnId, title: t.kanban.colTodo, color: 'border-[#ebdcb9]/45 bg-[#faf6ee]/10', label: language === 'en' ? 'Pending foraging queue' : 'Cola de acopio pendiente' },
              { id: 'in_progress' as ColumnId, title: t.kanban.colInProgress, color: 'border-amber-500/10 bg-amber-500/5', label: language === 'en' ? 'Tasks in maturation' : 'Labores en maduración' },
              { id: 'completed' as ColumnId, title: t.kanban.colCompleted, color: 'border-emerald-500/10 bg-emerald-500/5', label: language === 'en' ? 'Honeycombs filled with honey' : 'Celdas colmadas de miel' }
            ].map((column) => {
              const columnTasks = filteredTasks.filter((t) => t.columnId === column.id);

              return (
                <KanbanColumn key={column.id} column={column} tasks={columnTasks} language={language}>
                  <SortableContext
                          ...
                          columnTasks.map((task) => (
                            <SortableTaskItem
                              ...
                              onDelete={async () => {
                                ...
                                showToast(language === 'en' ? 'Task discarded from the hive' : 'Labor descartada del panal', 'warning');
                              }}
                            />
                          ))
                        )}
