...
const markdownRenderer = {
  heading(children: React.ReactNode, level: number) {
    ...
    return createElement(`h${level}`, { className: classes[level] || classes[3], key: undefined }, children);
  },
  paragraph(children: React.ReactNode) {
    return createElement('p', { className: 'my-1.5 text-xs text-stone-700 leading-relaxed', key: undefined }, children);
  },
  strong(children: React.ReactNode) {
    return createElement('strong', { className: 'font-bold text-[#100f0d]', key: undefined }, children);
  },
  ...
  codespan(code: string) {
    return createElement('code', {
      className: 'bg-[#faf6ee] text-[#e28800] px-1.5 py-0.5 rounded border border-[#ebdcb9] font-mono text-[10px]',
      key: undefined,
    }, code);
  },
  ...
  list(children: React.ReactNode, ordered: boolean) {
    ...
    return createElement(Tag, { className: 'list-disc ml-5 text-xs text-stone-700 my-1 space-y-1', key: undefined }, children);
  },
  listItem(children: React.ReactNode) {
    return createElement('li', { className: 'text-xs text-stone-700 my-1', key: undefined }, children);
  },
...
export const MarkdownNotesEditor: React.FC<MarkdownNotesEditorProps> = ({ taskId, initialValue = '', onClose }) => {
    ...
    <div className="w-full mt-3 bg-stone-50/80 border border-[#ebdcb9]/40 rounded-xl p-3 flex flex-col space-y-2 select-none animate-fade-in">
      ...
      <div className="w-full min-h-[90px] select-text">
        {mode === 'edit' ? (
          ...
          <p className="text-stone-400 italic p-1 text-xs">No hay notas escritas... Haz clic en Editar para comenzar.</p>
        ) : (
