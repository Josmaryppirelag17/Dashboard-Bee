...
function useTaskCardContext() {
  ...
  if (!context) {
    throw new Error('TaskCard compound elements must be encapsulated inside a <TaskCard> container.');
  }
  return context;
}
...
TaskCard.Checkbox = function TaskCardCheckbox() {
  ...
  return (
    ...
    >
      ...
      {task.completed && (
        <Check className="w-4 h-4 text-white stroke-[3.5] relative z-10" />
      )}
    </button>
  );
};
  ...
  return (
    <div className="flex-1 min-w-0 mx-3">
      ...
      <div className="flex flex-wrap items-center gap-1.5 mt-1.5">
        <Badge variant="neutral" className="text-[10px] px-1.5 py-0.5">{task.category}</Badge>
        <Badge variant={priorityMap[task.priority]} className="text-[10px] px-1.5 py-0.5">{priorityLabelMap[task.priority]}</Badge>
      </div>
    </div>
  );
};
