...
function SectionFallback() {
  ...
  return (
    <div className="flex flex-col items-center justify-center p-20 min-h-[400px]" role="status" aria-label="Cargando sección">
...
export default function App() {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (activeElement) {
        ...
        const isEditable = tagName === 'INPUT' || tagName === 'TEXTAREA' || activeElement.getAttribute('contenteditable') === 'true';
  ...
  const xpPercentage = useMemo(() => Math.min(Math.round((xp / xpNeeded) * 100), 100), [xp, xpNeeded]);
  ...
  const handleExportCSV = async () => {
    if (tasks.length === 0) {
      showToast(language === 'en' ? 'No tasks in the hive to export' : 'No hay labores en el panal para exportar', 'warning');
      return;
    }
    setIsExporting(true);
    try {
      ...
      showToast(language === 'en' ? 'Cells exported to CSV successfully!' : '¡Celdas exportadas a CSV con éxito!', 'success');
      useHiveStore.getState().unlockAchievement('honey-exporter');
    } catch (err) {
      console.error('CSV Extraction failed:', err);
      showToast(language === 'en' ? 'Error processing CSV export' : 'Error al procesar la exportación del CSV', 'warning');
    } finally {
  ...
  const handleClearCompleted = async () => {
    ...
    showToast(language === 'en' ? 'Cleared harvested tasks from the hive' : 'Se han vaciado las labores recolectadas del panal', 'success');
  };
  ...
  const metricsList = useMemo((): Metric[] => [
    {
      id: 'focus-time',
  ...
  ], [language, totalFocusMins, completedTasksCount, totalTasksCount, taskCompletionRate, totalPollenProduced, streakCount, t.metrics.focusMins]);
    ...
    <header className="h-[72px] bg-white/70 backdrop-blur-md border-b border-[#ebdcb9]/60 px-4 sm:px-8 flex items-center justify-between flex-shrink-0 z-20">
      <div className="flex items-center space-x-2 sm:space-x-3">
        ...
        <div className="flex items-center space-x-1.5 ml-2">
          <span className={`w-2 h-2 rounded-full ${session.isOnline ? 'bg-emerald-500 animate-pulse' : 'bg-stone-300'}`} />
          <span className="text-[9px] font-mono text-[#5c5449] font-bold hidden xl:inline" title={`ID: ${session.deviceId}`}>
      ...
      <div className="flex items-center space-x-2 sm:space-x-3">
        <button
          ...
          title={language === 'en' ? 'Import tasks from CSV backup' : 'Importar labores desde backup CSV'}
        >
          <Upload className="w-4 h-4 text-[#e28800]" />
          <span className="hidden sm:inline">{language === 'en' ? 'Import CSV' : 'Importar CSV'}</span>
        </button>
        <input
          ref={fileInputRef}
          type="file"
        ...
        >
          {isExporting ? <Loader2 className="w-4 h-4 animate-spin text-[#e28800]" /> : <Download className="w-4 h-4 text-[#e28800]" />}
          <span className="hidden sm:inline">{language === 'en' ? 'Export CSV' : 'Exportar CSV'}</span>
        </button>
        <button
          type="button"
          ...
          {language === 'en' ? (
            <>
              ...
              <ul className="text-xs text-[#5c5449] list-disc ml-5 mt-2 space-y-1">
                <li><strong>Swarm Progression</strong>: Complete chores and run timer flights to level up your Hive. Randomize bee personas anytime in the lower profile dashboard.</li>
                <li><strong>CSV Backups</strong>: Safeguard your tasks via "Export CSV" and restore datasets fluidly anytime through the newly integrated "Import CSV" module!</li>
                <li><strong>Core Metrics</strong>: Real-time monitoring of your status through hexagons with animated progress bars.</li>
                <li><strong>Task Depot (Kanban)</strong>: Fluidly manage daily targets by dragging items between columns.</li>
                <li><strong>Smart Search</strong>: Filter chores instantly by entering search queries in the top bar (shortcut: press <code className="bg-stone-100 border border-[#ebdcb9] px-1 rounded">/</code>).</li>
                <li><strong>Markdown Notes</strong>: Click any task title or journal icon to expand a rich text editor with hotkeys and live preview.</li>
              </ul>
            </>
          ) : (
            <>
              <p className="text-xs text-[#5c5449] leading-relaxed">
                ¡Bienvenido a <strong>BeeHive Productivity</strong>! Maximiza la producción de tu jornada:
              </p>
              <ul className="text-xs text-[#5c5449] list-disc ml-5 mt-2 space-y-1">
                <li><strong>Progresión de la Colmena</strong>: Completa labores y temporizadores para subir de nivel. Modifica tu nombre de abeja en la parte inferior cuando gustes.</li>
                <li><strong>Backups de Datos</strong>: Guarda tus labores vía "Exportar CSV" y restáuralas en cualquier momento con el nuevo cargador "Importar CSV" del panel general.</li>
                <li><strong>Celdas Principales</strong>: Supervisa en tiempo real tus métricas con los hexágonos con barras de progreso animadas.</li>
                <li><strong>Depósito de Labores (KanBan)</strong>: Gestiona objetivos diarios arrastrando las tareas entre columnas de forma fluida.</li>
                <li><strong>Búsqueda Inteligente</strong>: Filtra tareas o notas al instante escribiendo en la barra superior (atajo: presiona la tecla <code className="bg-stone-100 border border-[#ebdcb9] px-1 rounded">/</code>).</li>
                <li><strong>Notas Markdown</strong>: Haz clic en el título de cualquier tarea o su icono de diario para expandir un editor de texto con accesos rápidos y previsualizador en tiempo real.</li>
              </ul>
            </>
          )}
        </div>
      ...
      <Suspense fallback={<SectionFallback />}>
        {activeTab === 'dashboard' && (
          <div className="space-y-6 relative z-10">
            ...
            {/* Welcome Banner */}
            <div className="bg-gradient-to-r from-white via-white/90 to-[#faf6ee]/30 border border-[#ebdcb9] rounded-3xl p-6 relative overflow-hidden flex flex-col md:flex-row md:items-center md:justify-between gap-5 shadow-[0_4px_25px_rgba(110,80,45,0.03)] animate-fade-in">
              <div className="space-y-1.5 relative z-10">
                ...
                <h2 className="text-xl sm:text-2xl font-black tracking-tight text-[#100f0d]">
                  {language === 'en' ? 'Welcome to the swarm, ' : '¡Bienvenido al enjambre, '}<span className="text-[#e28800]">{userBeeName}!</span>
                </h2>
                ...
                <p className="text-xs text-[#5c5449] max-w-xl leading-relaxed">
                  {language === 'en' ? (
                    <>
                      Your hive tasks are organized. Rank: <strong className="text-[#e28800]">{hiveRank}</strong>. Completion rate: <strong className="text-[#e28800]">{taskCompletionRate}%</strong> and you have harvested <strong className="text-[#e28800]">{totalPollenProduced}u</strong> of fresh pollen.
                    </>
                  ) : (
                    <>
                      Tus labores del panal están organizadas. Categoría actual: <strong className="text-[#e28800]">{hiveRank}</strong>. Eficiencia de metas: <strong className="text-[#e28800]">{taskCompletionRate}%</strong> y has cosechado <strong className="text-[#e28800]">{totalPollenProduced}u</strong> de polen fresco.
                    </>
                  )}
                </p>
              </div>
            ...
            <div data-cv="auto" className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
              <div className="lg:col-span-6 bg-[#faf6ee]/70 border border-[#ebdcb9] rounded-3xl p-6.5 flex flex-col shadow-[0_4px_20px_rgba(110,80,45,0.02)]">
                <div className="flex flex-col flex-1 min-h-0">
                  ...
                  <div className="space-y-4 flex-1 min-h-0 overflow-y-auto custom-scrollbar pr-1">
                    {quests.map((q) => {
                      const qProgress = Math.min(Math.round((q.progress.current / q.progress.target) * 100), 100);
                      const isClaimed = claimedQuests.includes(q.id);
                      const canClaim = q.condition && !isClaimed;
                      return (
                        <div key={q.id} className={`bg-white/80 border rounded-2xl p-3.5 flex items-start gap-3 shadow-xs ${isClaimed ? 'border-emerald-300/40 opacity-70' : 'border-[#ebdcb9]/40'}`}>
                          <div className="mt-1 w-4.5 h-4.5 shrink-0 rounded-lg flex items-center justify-center text-[9px] font-black border border-[#ebdcb9] bg-[#faf6ee]">
                            <span className={`${q.condition ? 'text-[#e28800]' : 'text-stone-300'}`}>
                              {isClaimed ? '✓' : q.condition ? '!' : q.progress.current}
                            </span>
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="flex justify-between items-center">
                              <h4 className={`text-xs font-black truncate ${isClaimed ? 'text-emerald-700 line-through' : q.condition ? 'text-[#100f0d]' : 'text-[#5c5449]'}`}>
                                {q.title}
                              </h4>
                              <span className="text-[10px] text-emerald-700 font-bold font-mono">+{q.xpReward} XP</span>
                            </div>
                            <p className="text-[10px] text-[#5c5449] leading-tight mb-2">{q.desc}</p>
                            <div className="flex items-center gap-2">
                              ...
                              {canClaim ? (
                                ...
                                <span className="text-[9px] font-bold text-stone-500 font-mono shrink-0">
                                  {isClaimed ? (language === 'en' ? 'Done!' : '¡Hecho!') : `${q.progress.current}/${q.progress.target}`}
                                </span>
                              )}
                            </div>
                          </div>
                ...
                <div className="text-[9px] text-[#5c5449]/70 font-semibold italic text-center mt-4 shrink-0">
                  {language === 'en' ? 'Complete quests to earn XP and level up. Harder quests unlock at higher levels!' : 'Completa misiones para ganar XP y subir de nivel. ¡Misiones más difíciles se desbloquean en niveles superiores!'}
                </div>
              </div>
              ...
              <div className="lg:col-span-6 bg-white border border-[#ebdcb9] rounded-3xl p-6.5 flex flex-col justify-between shadow-[0_4px_20px_rgba(110,80,45,0.02)]">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {(() => {
                      const achievementsList = [
                        { id: 'first-tasks', title: language === 'en' ? 'First Drop' : 'Primera Gota', desc: language === 'en' ? 'Seed initial tasks list in IndexDB' : 'Inicializa tus primeras labores en IndexedDB', icon: <Sparkle className="w-5 h-5 text-amber-500" /> },
                        { id: 'wax-creator', title: language === 'en' ? 'Wax Sculptor' : 'Constructor de Celdas', desc: language === 'en' ? 'Enter a custom task to the active hive' : 'Ingresa una tarea personalizada al panal', icon: <CheckCircle className="w-5 h-5 text-amber-500" /> },
                        { id: 'harvest-honey', title: language === 'en' ? 'Honey Collector' : 'Recolector de Miel', desc: language === 'en' ? 'Harvest tasks into completed cells' : 'Muda las labores a celdas colmadas de miel', icon: <Trophy className="w-5 h-5 text-amber-500" /> },
                        { id: 'pomodoro-first', title: language === 'en' ? 'Initial Flight' : 'Primer Vuelo', desc: language === 'en' ? 'Complete 1 focus session flight' : 'Realiza tu primera sesión de vuelo de concentración', icon: <Flame className="w-5 h-5 text-orange-500" /> },
                        { id: 'mighty-forager', title: language === 'en' ? 'Mighty Forager' : 'Súper Polinizador', desc: language === 'en' ? 'Register a task with 5-effort workload' : 'Completa una labor de alta demanda (5 polen)', icon: <Zap className="w-5 h-5 text-amber-500" /> },
                        { id: 'honey-exporter', title: language === 'en' ? 'Swarm Exporter' : 'Sindicato de Exportadoras', desc: language === 'en' ? 'Execute a backup data CSV shipment' : 'Transfiere un cargamento de datos a formato CSV', icon: <Download className="w-5 h-5 text-emerald-600" /> }
                      ];
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              <p className="text-[8.5px] text-[#5c5449] truncate leading-tight" title={ach.desc}>
                                {ach.desc}
                              </p>
                            </div>
                            {isUnlocked && (
                ...
                <div className="mt-4 bg-[#faf6ee] rounded-xl p-2.5 border border-[#ebdcb9]/40 text-center text-[9px] font-bold text-[#e28800] uppercase tracking-wide">
                  {language === 'en' ? '🏆 UNLOCK ACHIEVEMENTS TO HARVEST +100 XP REWARDS!' : '🏆 ¡DESBLOQUEA LOGROS PARA COSECHAR BONOS DE +100 XP!'}
                </div>
              </div>
            </div>
            ...
            <div>
              <div className="flex items-center space-x-1.5 mb-3.5 select-none">
                ...
                <h4 className="text-[10px] font-bold text-[#5c5449] uppercase tracking-widest">
                  {language === 'en' ? 'Production Hive Metrics' : 'Métricas del Panal de Producción'}
                </h4>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4.5">
                {metricsList.map((metric) => (
                  <MetricCard
                    ...
                    progressPercent={
                      metric.id === 'focus-time' ? Math.min(100, (totalFocusMins / 600) * 100) :
                      metric.id === 'tasks-ratio' ? taskCompletionRate :
                      metric.id === 'pollen-produced' ? Math.min(100, (totalPollenProduced / 50) * 100) :
                      metric.id === 'colony-streak' ? Math.min(100, (streakCount / 10) * 100) : 75
                    }
                  />
                ))}
              </div>
            ...
            <div className="bg-white/75 backdrop-blur-md border border-[#ebdcb9]/60 rounded-3xl p-6.5 shadow-[0_10px_35px_rgba(110,80,45,0.03)]">
              <div className="mb-6 select-none">
                <h3 className="text-lg font-bold text-[#100f0d]">
                  {language === 'en' ? 'Consolidated Collection Report' : 'Informe de Acopio Consolidado'}
                </h3>
                <p className="text-xs text-[#5c5449]">
                  {language === 'en' ? 'Complete analysis of nectar yield generated by the swarm daily' : 'Análisis completo del rendimiento de néctar generado por el enjambre diario'}
                </p>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
                ...
                <div className="space-y-4">
                  <h4 className="text-xs font-bold text-[#5c5449] uppercase tracking-widest pb-2 border-b border-[#ebdcb9]">
                    {language === 'en' ? 'Completed Work Milestones' : 'Hitos de Trabajo Completados'}
                  </h4>
                  <div className="space-y-3">
                    <div className="flex justify-between items-center bg-[#faf6ee] p-3.5 rounded-2xl border border-[#ebdcb9] text-xs font-semibold text-[#100f0d]">
                      <span>{language === 'en' ? 'Consistency Status' : 'Grado de Constancia'}</span>
                    ...
                    <div className="flex justify-between items-center bg-[#faf6ee] p-3.5 rounded-2xl border border-[#ebdcb9] text-xs font-semibold text-[#100f0d]">
                      <span>{language === 'en' ? 'Total Focus Sessions' : 'Sesión de Foco Total'}</span>
